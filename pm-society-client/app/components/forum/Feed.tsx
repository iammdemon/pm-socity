// PostFeed.tsx - Enhanced with Twitter/LinkedIn design
"use client";
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-white dark:bg-black rounded-xl border border-gray-200 dark:border-gray-800 p-4 animate-pulse">
  ...
  return (
    <div className="space-y-4">
      ...
      {posts.map((post) => {
        ...
        return (
          <article key={post.topicId} className="bg-white dark:bg-black rounded-xl border border-gray-200 dark:border-gray-800 hover:shadow-lg transition-all duration-200">
            <div className="p-4">
              {/* Post Header */}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      @{post.author?.email?.split("@")[0] || "user"} · {formatRelativeTime(post.createdAt)}
                    </p>
                  </div>
                </div>
                  ...
                  <button
                    ...
                    className={`flex items-center space-x-2 px-3 py-1.5 rounded-full transition-colors ${
                      reacted 
                        ...
                        : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900 hover:text-red-500"
                    }`}
                    disabled={isReactingThisPost}
                  >
                    <Heart className={`w-4 h-4 ${reacted ? "fill-current" : ""} ${isReactingThisPost ? "animate-pulse" : ""}`} />
                    <span className="text-sm">{post.reactionCount || 0}</span>
                  </button>
                  <Link href={`/dashboard/forum/${post.topicId}`}>
                    <button className="flex items-center space-x-2 px-3 py-1.5 rounded-full text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900 hover:text-blue-500 transition-colors">
                      <MessageCircle className="w-4 h-4" />
                      <span className="text-sm">{post.replyCount || 0}</span>
                    </button>
                  </Link>
                  <button className="flex items-center space-x-2 px-3 py-1.5 rounded-full text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900 hover:text-green-500 transition-colors">
                    <Share2 className="w-4 h-4" />
                  </button>
                </div>
                <button className="text-gray-500 dark:text-gray-400 hover:text-blue-500 transition-colors p-1.5">
