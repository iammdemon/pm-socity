      ...
      <main className="min-h-screen bg-[#ECE8E1]">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-12 sm:py-16">
          <div className="flex justify-center mb-8 sm:mb-12">
            <h2
              className={`${bonVivant.className} text-xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-black`}
            >
              Latest Articles
            </h2>
          </div>
          ...
          {isLoading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {[...Array(6)].map((_, idx) => (
                <div
                  key={idx}
                  className="h-64 sm:h-80 bg-gray-200 animate-pulse rounded-2xl"
                />
              ))}
            </div>
          )}
          ...
          {isError && (
            <div className="text-center text-red-600 font-semibold py-12 sm:py-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {blogs.map((post, index) => (
                <article
                  ...
                  className={`group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-500 transform hover:-translate-y-1 cursor-pointer ${
                  ...
                  <div className="relative h-48 sm:h-56 md:h-64">
                    ...
                    {post.trending && (
                      <div className="absolute top-3 right-3">
                        <span className="px-2 py-1 bg-black/80 text-white text-xs sm:text-sm font-semibold rounded-full flex items-center gap-1">
                  ...
                  <div className="p-4 sm:p-6">
                    <div className="flex gap-2 mb-2 sm:mb-3">
                      {post.tags.slice(0, 2).map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-1 bg-gray-100 text-gray-600 text-xs sm:text-sm font-medium rounded-lg"
                        >
                    ...
                    <h2 className="text-base sm:text-lg md:text-xl font-bold text-black mb-2 group-hover:text-gray-700 line-clamp-2 leading-snug">
                      {post.title}
                    </h2>
                    <p className="text-gray-600 text-sm sm:text-base mb-3 sm:mb-4 line-clamp-3">
                      {post.excerpt}
                    </p>
                    <div className="border-t border-gray-100 pt-3 sm:pt-4 flex items-center justify-between text-xs sm:text-sm text-gray-500">
                      <div className="flex items-center gap-2">
                        <FiCalendar className="w-3 h-3 sm:w-4 sm:h-4" />
                        <span>{formatDate(post.createdAt)}</span>
                      </div>
                      <FiArrowRight className="w-4 h-4 sm:w-5 sm:h-5 text-black group-hover:translate-x-1 transition-transform duration-300" />
                    </div>
                  </div>
                </article>
              ))}
